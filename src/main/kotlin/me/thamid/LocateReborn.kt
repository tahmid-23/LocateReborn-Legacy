 ...
 */
fun queryData(input: BufferedReader, out: PrintWriter) {
...
private fun printStudentCourses(studentList: Element, cookies: Map<String, String>, out: PrintWriter, courseStudentMap: MutableMap<String, String>) {
    for (student in studentList.children()) {
        ...
        getCourseInfo(courseLink.attr("href"), cookies)?.getElementsByClass("my-courses-item-list")?.first()?.let {
            val name = courseLink.text()
            out.println(name)
            out.println("Mod, Monday, Tuesday, Wednesday, Thursday, Friday")

            val courses = mutableListOf<Course>()
            addCourseInfo(name, it, courseStudentMap, courses)
 ...
 */
private fun addCourseInfo(name: String, courseList: Element, courseStudentMap: MutableMap<String, String>, courses: MutableList<Course>) {
    for (course in courseList.children()) {
        ...
        val courseObject = Course(courseName)

        if (courseStudentMap[courseId] == null) courseStudentMap[courseId] = ",\"$courseName\",$name" else courseStudentMap[courseId] += ",$name"
 ...
 *
 * @param userId The student's user ID as represented within the URL
 * @param cookies Cookies associated with the current login
 * @return The course's information
 */
private fun getCourseInfo(userId: String, cookies: Map<String, String>): Document? {
    val request = Jsoup.connect("https://bca.schoology.com$userId/courses/list")
            .method(Connection.Method.GET)
            .cookies(cookies)
            .ignoreHttpErrors(true)
            .data("destination", "${userId.substring(1)}/info")
